...
export default function NotesPanel({ targetFolder, subdomains }) {
  ...
  const saveNote = async () => {
    ...
    try {
      await axios.post(`${API}/recon/notes`, {
        target_folder: targetFolder, subdomain: selected, note: text,
      });
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      setNotes(prev => { const n = { ...prev }; delete n[deleteTarget]; return n; });
      if (selected === deleteTarget) { setSelected(null); setText(""); setTag(""); }
      setDeleteTarget(null);
    } catch {}
  ...
  const filtered = subdomains.filter(s =>
    ...
    (!filterTag || notes[s]?.tag === filterTag)
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div style={{ background: "#0a0a0a", border: "1px solid #1f2937", borderRadius: 12, padding: "14px 18px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          <p style={{ fontFamily: "monospace", fontSize: 10, color: "#374151", marginTop: 2 }}>
            {withNotes.length} subdomains with notes · {targetFolder ? "saving to target folder" : "not connected"}
          </p>
        </div>
        <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
          <button onClick={() => setFilterTag("")} style={{
            ...
            background: !filterTag ? "#1f2937" : "transparent", border: `1px solid ${!filterTag ? "#374151" : "#1f2937"}`,
            color: !filterTag ? "#e5e7eb" : "#374151",
          }}>All</button>
          {TAGS.map(t => tagCounts[t.id] > 0 && (
            <button key={t.id} onClick={() => setFilterTag(filterTag === t.id ? "" : t.id)} style={{
              fontFamily: "monospace", fontSize: 9, padding: "2px 8px", borderRadius: 4, cursor: "pointer",
      ...
      {deleteTarget && (
        <div style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.75)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#0a0a0a", border: "1px solid #7f1d1d", borderRadius: 12, padding: 24, maxWidth: 420, width: "90%" }}>
            ...
            <p style={{ fontFamily: "monospace", fontSize: 11, color: "#374151", marginBottom: 20 }}>This will permanently delete the note. This cannot be undone.</p>
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
              <button onClick={() => setDeleteTarget(null)} style={{ padding: "8px 18px", background: "#111827", border: "1px solid #374151", borderRadius: 8, color: "#9ca3af", fontFamily: "monospace", fontSize: 12, cursor: "pointer" }}>Cancel</button>
              <button onClick={executeDelete} disabled={deleting} style={{ padding: "8px 18px", background: "#7f1d1d", border: "1px solid #ef4444", borderRadius: 8, color: "#fca5a5", fontFamily: "monospace", fontSize: 12, cursor: "pointer" }}>
        ...
        <div style={{ gridColumn: "span 2", background: "#0a0a0a", border: "1px solid #1f2937", borderRadius: 12, padding: 14, display: "flex", flexDirection: "column", gap: 8, overflow: "hidden" }}>
          ...
          <div style={{ overflowY: "auto", flex: 1, display: "flex", flexDirection: "column", gap: 3 }}>
            {filtered.map(sub => {
              ...
              const tagInfo = TAGS.find(t => t.id === notes[sub]?.tag);
              const isSel   = selected === sub;
              return (
                <div key={sub} onClick={() => selectSub(sub)} style={{
                  display: "flex", alignItems: "center", justifyContent: "space-between",
                  padding: "7px 10px", borderRadius: 6, cursor: "pointer",
                  background: isSel ? "#0a1c0e" : "#000",
                  border: `1px solid ${isSel ? "#166534" : tagInfo ? tagInfo.color + "33" : "#111827"}`,
                  transition: "all 0.12s",
                }}
                  onMouseEnter={e => { if (!isSel) e.currentTarget.style.background = "#0d1117"; }}
                  onMouseLeave={e => { if (!isSel) e.currentTarget.style.background = "#000"; }}
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      {tagInfo && <div style={{ width: 5, height: 5, borderRadius: "50%", background: tagInfo.color, flexShrink: 0 }} />}
                      <span style={{ fontFamily: "monospace", fontSize: 11, color: isSel ? "#86efac" : "#67e8f9", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        {sub}
                      </span>
                    </div>
                    {hasNote && (
                  ...
                  {hasNote && (
                    <button onClick={e => { e.stopPropagation(); setDeleteTarget(sub); }}
                      style={{ marginLeft: 6, flexShrink: 0, background: "none", border: "none", color: "#374151", fontSize: 12, cursor: "pointer", padding: "2px 4px", borderRadius: 4 }}
            ...
            <>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                  <a href={`https://${selected}`} target="_blank" rel="noopener noreferrer"
                    style={{ fontFamily: "monospace", fontSize: 12, color: "#67e8f9", textDecoration: "none", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}
                ...
                <div style={{ display: "flex", gap: 6, alignItems: "center", flexShrink: 0 }}>
                  ...
                  {notes[selected]?.text && (
                    <button onClick={() => { setEditMode(m => !m); setTimeout(() => textareaRef.current?.focus(), 50); }} style={{
                      padding: "4px 10px", borderRadius: 6, cursor: "pointer", fontFamily: "monospace", fontSize: 10,
              ...
              {notes[selected]?.text && !editMode ? (
                <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  }}>{notes[selected].text}</div>
                  <button onClick={() => { setEditMode(true); setTimeout(() => textareaRef.current?.focus(), 50); }} style={{
                    alignSelf: "flex-end", padding: "8px 20px", background: "#0c1a2e",
                    border: "1px solid #3b82f6", borderRadius: 8, color: "#60a5fa",
                ...
                <>
                  ...
                  <div style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center" }}>
                    ...
                    <button onClick={() => setTag("")} style={{
                      fontFamily: "monospace", fontSize: 9, padding: "2px 8px", borderRadius: 4, cursor: "pointer",
                      background: !tag ? "#1f2937" : "transparent", border: `1px solid ${!tag ? "#374151" : "#1f2937"}`,
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      ...
                      {notes[selected]?.text && (
                        <button onClick={() => { setText(notes[selected].text); setTag(notes[selected].tag || ""); setEditMode(false); }}
                          style={{ fontFamily: "monospace", fontSize: 10, color: "#374151", background: "none", border: "none", cursor: "pointer" }}>
                          ✕ Discard
                        </button>
                      )}
                    </div>
                    <button onClick={saveNote} disabled={saving || !text.trim()} style={{
                      padding: "8px 20px",
                      background: saved ? "#052e16" : saving ? "#0d1117" : "#166534",
                      border: `1px solid ${saved ? "#4ade80" : saving ? "#1f2937" : "#166534"}`,
                      borderRadius: 8, color: saved ? "#4ade80" : saving ? "#374151" : "#fff",
                      fontFamily: "monospace", fontSize: 12,
                      cursor: saving || !text.trim() ? "not-allowed" : "pointer", transition: "all 0.2s",
                    }}>
      ...
      {withNotes.length > 0 && (
        <div style={{ background: "#0a0a0a", border: "1px solid #1f2937", borderRadius: 12, padding: "14px 18px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {withNotes.map(sub => (
              <div key={sub} onClick={() => selectSub(sub)} style={{
                background: "#000", border: "1px solid #111827", borderRadius: 8, padding: "10px 14px",
